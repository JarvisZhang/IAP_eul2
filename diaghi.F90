...
!!   use precision
   ...
   use pmgrid,     only :iam, npr_y, npr_z, myid_z, beglatdyn, endlatdyn,beglev,endlev,endlevp
#if (defined SPMD)
...
!------------------------------------------------------------------------------------------------
!  print*, '------ at beginning of sub. diaghi ---------'
  if(npr_z.gt.1) then
#if (defined SPMD)
...
!          H0B £½ Rd * TMSA(PSB)/PSB,   TMSA: temperature of standard atmosphere 
      END DO
!!      if(j==10) print*, '------ at beginning of sub. diaghi ---------'
!!      if(j==10) print*, 'PT(10,10) =', PT(10,10)
   END DO
   DO J = beglatdyn, endlatdyn
      DO K = 1 ,NL
         DO I = 1 ,NX
            deltap(I,K,J) = PMTOP / PLY (I,K,J )
            tmp = (PT(I,J)*SIGL(K))*DSIG(K)
            if (abs(tmp) < 1e-15) then
               print*, 'tmp =', tmp
            end if
!!            print*, 'i,k,j =', i, j, k
!!            print*, 'b0 =', b0, ' deltap(I,K,J) =', deltap(I,K,J)
!!            print*, 'TT1(I,K,J) =', TT1(I,K,J), ' PT(I,J) =', PT(I,J)
!!            print*, 'SIGL(K) =', SIGL(K), ' DSIG(K) =', DSIG(K)
!!            if(j==10) print*, 'PT(10,10) =', PT(10,10)
