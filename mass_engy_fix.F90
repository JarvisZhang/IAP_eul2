...
subroutine mass_engy_fix (ztodt,   &
                            lat,   &
                            nlon,              &
                          u3m1,      &
                          u3,     v3m1,   v3,                &
...
! 
! Update: 2011.03.25 by Zhang He, not do the energy fixer while running with ideal physics (H-S forcing)
!-----------------------------------------------------------------------
   ...
   use constituents, only: pcnst, pnats, qmin, cnst_cam_outfld, &
...
!
   ...
   real(r8), intent(in) :: q3m1(plon,beglev:endlev,pcnst+pnats)
! ========================= zhh 2008.9.11 ===============================
...
!
   ...
   real(r8), intent(inout) :: q3(plon,beglev:endlev,pcnst+pnats)
...
!
   ...
   real(r8) :: q3tmp(nlon,plev,pcnst+pnats)
...
!=========================== zhh 2008.6.17 =========================
!!   if (lat == 65) then
!!      print*, 'after fix & before qneg3'
!!      print*, 'q3(89,10,1,65) =', q3(89,10,1)
!!   end if 
!=========================== zhh 2008.6.17 =========================
#if (defined SPMD)
   if(npr_z.gt.1) then
...
#endif
    q3tmp(1:plon,beglev:endlev,1:(pcnst+pnats))=q3(1:plon,beglev:endlev,1:(pcnst+pnats))
#if (defined SPMD)
   if(npr_z.gt.1) then
    call parcollective3d( comm_z, sumop,plon ,plev,pcnst+pnats ,q3tmp)
   endif
#endif
   call qneg3 ('MASS_ENGY_FIX',lat   ,nlon    ,plon   ,plev    , &
               1, pcnst+pnats,qmin ,q3tmp(1,1,1))
     q3(1:plon,beglev:endlev,1:(pcnst+pnats))=q3tmp(1:plon,beglev:endlev,1:(pcnst+pnats))
...
!
   do m=1,pcnst
      if ( cnst_cam_outfld(m) ) then
         call outfld(tottnam(m),ta(:,:,m),plon   ,lat     )
      end if
   end do
!
...
!
   do m=1,pcnst
      if ( cnst_cam_outfld(m) ) then
         call outfld (tendnam(m),qtend(:,:,m),plon,lat)
