...
!------------------------------------------------------------------------------------------------
   ...
   use IAP_grid,   only : NX, NY, NL, IB, IE, JB, JE, period,ex
   ...
   use pmgrid,     only : beglatdyn, endlatdyn,beglev,endlev,plat,iam
...
!!       do i=1,nx
!! test        if(j.eq.179.and.beglev.le.7.and.endlev.ge.7) print*,'test TT',CCT(i,7,j),DT(i,k,j),&
!!i,k,j,iam
!!       end do
!!  end do
    call t_startf('uvtp')
...
!!  if(j.eq.180.and.beglev.le.7.and.endlev.ge.7) print*,'test Pstar1',Ps2(i,j),&
!!i,k,j,iam,istar, Psa(i,j),P(i,j),CCP(i,j),PSB(i,j),DPSA(i,j)
