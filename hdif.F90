...
! Completed : 2006.4.30 
! Update : 2007.4.16, ZhangHe, add the dumb parameter 'deltat' to sub. HDIFUS
!          2007.4.25, ZhangHe, add the dumb parameter 'PLY, U, V, T, Q' to sub. HDIFUS 
!          2008.4.21, ZhangHe, delete diffusion of q
...
!-----------------------------------------------------------------------------------------------
   ...
   use pmgrid,    only: beglatdyn, endlatdyn, loc_JB, loc_JE, iam, beglatdynex, endlatdynex,beglev,endlev,endlevp
#if (defined SPMD)
...
!JJR         call mpi_move_left( VK(1,endlatdyn), VK(1,beglatdyn-1),NX)
!JJr         call mpi_move_right(  UK(1,beglatdyn), UK(1,endlatdyn+1),NX)
!!         call mpi_move_left( QK(1,endlatdyn), QK(1,beglatdyn-1),NX)
!!         call mpi_move_right(  QK(1,beglatdyn), QK(1,endlatdyn+1),NX)
!jjr         call mpi_move_right( ROT(1,beglatdyn),ROT(1,endlatdyn+1),NX)
!jjr         call mpi_move_right(  VR(1,beglatdyn), VR(1,endlatdyn+1),NX)
!jjr         call mpi_move_left( TK(1,endlatdyn), TK(1,beglatdyn-1),NX)
!jjr         call mpi_move_right(  TK(1,beglatdyn), TK(1,endlatdyn+1),NX)
#endif
!
...
!jjr         call mpi_move_right(  DT(1,beglatdyn), DT(1,endlatdyn+1),NX)
!jjr         call mpi_move_left( DS(1,endlatdyn), DS(1,beglatdyn-1),NX)
#endif
!
! Deduce D from DT & DS
!
...
!jjr         call mpi_move_left(D(1,endlatdyn),D(1,beglatdyn-1),NX)
#endif
!
!         DO J = loc_JB, loc_JE
         DO J = beglatdyn, endlatdyn
...
!jjr         call mpi_move_left(RLNT(1,endlatdyn),RLNT(1,beglatdyn-1),NX)
!jjr         call mpi_move_left(RDLT(1,endlatdyn),RDLT(1,beglatdyn-1),NX)
!jjr         call mpi_move_left(  DB(1,endlatdyn),  DB(1,beglatdyn-1),NX)
!jjr         call mpi_move_right(   DA(1,beglatdyn),  DA(1,endlatdyn+1),NX)
#endif
!------------------------------------------------------------------------------		 	 
!
