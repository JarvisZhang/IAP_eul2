...
!   03.10.31   Mirin      Removed unneeded ghostings, comm. patterns
!   03.11.11   Mirin      Added mod_method (opt. arg. parpatterncreate)
!
! !BUGS:
...
!jjr      call setrig
!jjr      call set_eta
!jjr      call dynpkg_init
!jjr      call d_split(nsplit_in)
#if defined( SPMD )
      call spmd_vars_init()
#endif
      return
...
! !USES:
   use pmgrid, only: mod_transpose
   use spmd_dyn, only : ghostpe_yz,                                     &
                        ghostpe1_yz, m_ttrans, q_ttrans, r_ttrans,      &
                        ijk_yz_to_xy, ijk_xy_to_yz,                     &
                        ikj_xy_to_yz,ikj_yz_to_xy, q_to_qxy, qxy_to_q,               &
                        pexy_to_pe,pe_to_pexy, pkxy_to_pkc, r_to_rxy, rxy_to_r
...
!
        ...
        call parpatterncreate(commglobal, strip3dxyz, strip3kxyz, &
                              ijk_yz_to_xy, mod_method=mod_transpose)
        call parpatterncreate(commglobal, strip3kxyz, strip3dxyz, &
                              ijk_xy_to_yz, mod_method=mod_transpose)
        call parpatterncreate(commglobal, strip3kxzy, strip3dxzy, &
                              ikj_xy_to_yz, mod_method=mod_transpose)
        call parpatterncreate(commglobal, strip3dxzy, strip3kxzy, &         !jjr add ikj_yz_to_xy
                              ikj_yz_to_xy, mod_method=mod_transpose)
        ...
        call parpatterncreate(commglobal, strip3kxzyp, ghostpe_yz, &
                              pexy_to_pe, mod_method=mod_transpose )
        call parpatterncreate(commglobal, ghostpe_yz, strip3kxzyp, &
                              pe_to_pexy, mod_method=mod_transpose )  !jjr add pe_to_pexy

        call parpatterncreate(commglobal, strip3kxyzp, ghostpe1_yz,&
                              pkxy_to_pkc, mod_method=mod_transpose )
...
! !USES:
   use spmd_dyn, only : ghostpe_yz, ghostpe1_yz,                        &
