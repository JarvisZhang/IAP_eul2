...
! Completed: 2005.8.25
! Update: 2007.4.23, ZhangHe, deleted the computation of 'DCB' in sub. DIAGBB
...
! ========================= test ==========================
   use pmgrid,     only: masterproc, iam,beglev,endlev,endlevp
! ===================== zhh 2008.6.12 =====================
         ...
         DO K = beglev ,endlev
            DO I = 1 ,NX
               ...
               if ( KPK == 0 ) then 
                  ...
                  WPI       = ONE - WPJ           ! REAL  WPI
                  KPI       = KPK + 1
           if(kpi.gt.NA.or.KPI.LT.1) print*,'error',kpi,ply(i,k,j),i,k,j
         ...
         do K = beglev ,endlev+1
            do I = 1 ,NX
               ...
               if ( KPK == 0 ) then 
                  ...
                  WPI      = ONE - WPJ           ! REAL  WPI
                  KPI      = KPK + 1
             if(kpi.gt.NA) print*,'error',kpi,pin(i,k,j),i,k,j
                  GHI0(I,K,J) = WPI*HBB (KPK) + WPJ*HBB (KPI)       ! GHI0(P) = phi(P)
               end if 
            end do
         end do
