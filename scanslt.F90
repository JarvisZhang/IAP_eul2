...
!-----------------------------------------------------------------------
!
! $Id: scanslt.F90,v 1.9.2.8 2005/03/02 20:52:32 bundy Exp $
!
!-----------------------------------------------------------------------
   use shr_kind_mod, only: r8 => shr_kind_r8
   use pmgrid
!jjr,       only: plon, plat, plev, beglat, endlat, beglatdyn, endlatdyn,  &
!                           plevp, masterproc, iam              !!zhh
...
!wjp 2011.04.13 integer, public :: numlatsex  ! number of latitudes owned by a given proc extended grid

#if ( ! defined SPMD )
   parameter (beglatex = 1)
   parameter (endlatex = platd)
!wjp 2011.04.13 parameter (numlatsex= platd)
#endif
...
!
   ...
   etamid(:) = pmid(lat,:)
   etaint(:) = pint(lat,:)
   if ( any(etamid == 0.0) ) call endrun('etamid == 0')
   if ( any(etaint == 0.0) ) call endrun('etaint == 0')
#endif
!
...
!!   use dynconst,    only: ra
   use physconst,    only: ra   !zhh 2008.1.8
   use prognostics, only: hadv, qminus
...
! Copy dynamics data into SLT advection structure
   call da_coupling( cwava, etadot, adv_state )
...
!u3,v3,qminus,need test!!!!!
      ...
      call mp_send3d(dest,src,plond,plev,platd,1,plond,beglev,endlev,beglatex,endlatex,&
                      1,plond,beglev,endlev,beglatex+3,beglatex+5,adv_state%u3)     
      call mp_recv3d(src,plond,plev,platd,1,plond,beglev,endlev,beglatex,endlatex,&
      ...
      call mp_recv3d(src,plond,plev,platd,1,plond,beglev,endlev,beglatex,endlatex,&
       ...
       call mp_send3d( dest, src, plond,plev*pcnst , platd,                      &
      ...
      call mp_recv3d( src, plond,plev*pcnst,platd,                             &
                      1, plond, beglev,(endlev-beglev+1)*pcnst+beglev-1,beglatex, endlatex,       &
...
!       print*,'tst 53',qxl(i,k,l,lat),i,k,l,lat
...
! Call slt interface routine.
  ...
  do l=1,pcnst
    ...
    do lat=beglat,endlat
      ...
      call mp_sendirr(dummy3, ikj_yz_to_xy%SendDesc,              &
                      ikj_yz_to_xy%RecvDesc, dummy3xy)
      call mp_recvirr(dummy3xy, ikj_yz_to_xy%RecvDesc )
    do lat=beglatxy,endlatxy
      ...
      call mp_sendirr(etadot, pe_to_pexy%SendDesc,              &
                      pe_to_pexy%RecvDesc, etadotxy)
      call mp_recvirr(etadotxy, pe_to_pexy%RecvDesc )
      call mp_sendirr(sigmp, ikj_yz_to_xy%SendDesc,              &
                      ikj_yz_to_xy%RecvDesc, sigmpxy)
      call mp_recvirr(sigmpxy, ikj_yz_to_xy%RecvDesc )
#endif
   else    
   fhrxy=fhr
   sigmpxy=sigmp
...
#if (defined SPMD)
      call mp_sendirr(sigmpxy, ikj_xy_to_yz%SendDesc,              &
                      ikj_xy_to_yz%RecvDesc, sigmp)
      call mp_recvirr(sigmp, ikj_xy_to_yz%RecvDesc )
...
!
!----------------------------------------------------------------------- 
   use prognostics, only: u3, v3, q3, qminus, n3m1, n3
! ====================================== zhh ==================================
...
!!               q3(i,k,c,j,n3) = qfcst(i,k,c,j)   !zhh 
!!            end do
         end do
      end do
   end do
...
!
   ...
   integer jdry                  ! latitude index for pdeld
...
! ====================================== zhh ==================================
   real(r8) :: UQ(NX,beglev:endlev,beglatdyn:endlatdyn), &
      VQ(NX,beglev:endlev,beglatdynex:endlatdyn),WQ(NX,beglev:endlevp,beglatdyn:endlatdyn)  !zhh 2008.6.16
...
!
!wjp 2011.04.16 !$OMP PARALLEL DO PRIVATE (C, LAT)
   do c=1,pcnst
      do lat=1,plat
         hw1lat(c,lat) = 0.0
      end do
...
!
      call plevs00 (nlon(lat),plon,plev,ps(1,lat,n3m2), pint, pmid, pdel)
!
! Calculate mass of moisture in field being advected by slt. (hw1lat)
!

!  q3     is plon,plev,pcnst+pnats,beglat:endlat,ptimelevs
...
!!                   hw1lat(1,lat),nlon(lat), q3(1,1,1,lat,n3m2), lat, pdeld(:,:,jdry,n3m2 ))
      call qmassarun (cwava(lat),w(irow) ,qminus(1,beglev,1,lat),pdel    , &
                   hw1lat(1,lat),nlon(lat), q3(1,beglev,1,lat,n3m1), lat, pdeld(:,:,jdry,n3m1 ))
       
   end do
!         do k=beglev,endlev
!             do i=1,nlon
!        print*,'test pdel',pdel(i,k),i,k
!               do lat=beglat,endlat
!       print*,'test qmassrun',q3(i,k,1,lat,n3m1),i,k,lat
!             end do
!         end do
!   end do
...
!zhh 2010.8.12            UVW0(I,K,J,1) = U (I,K,J)
!zhh 2010.8.12            UVW0(I,K,J,2) = V (I,K,J)
!zhh 2010.8.12            UVW0(I,K,J,3) = WS(I,K,J)
         end do
      end do
   end do
   do j = beglatdyn, endlatdyn
...
!   call mpi_move_left(VQ(1,1,endlatdyn), VQ(1,1,beglatdyn-1),plon*plev)
#endif
...
!
!wjp 2011.04.16 !$OMP  PARALLEL DO PRIVATE (J, K, M, NLOND)
