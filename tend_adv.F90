...
!------------------------------------------------------------------------------------------------
   ...
   use pmgrid,    only: beglatdyn, endlatdyn, loc_JB, loc_JE,beglev,endlev,iam,npr_y,&
  ...
  use mod_comm, only: mp_send3d, mp_recv3d,mp_send3d_2, mp_recv3d_2
   ...
   IMPLICIT NONE
!----------------------------------Local workspace-----------------------------------------------
   real(r8) :: UZ(NX,beglev:endlev+2,beglatdyn:endlatdyn),VZ(NX,beglev:endlev+2,beglatdyn:endlatdyn),&
                    TZ(NX,beglev:endlev+2,beglatdyn:endlatdyn)   ! extended variables in vertical layer
   real(r8) :: WW(NX,NY)                    ! wjp
   ...
   real(r8):: sbuf(NX,2*(endlev-beglev+1)+4)
   real(r8):: sbuf1(NX,4*(endlev-beglev+1)+4)
   real(r8)::rbuf(NX,2*(endlev-beglev+1)+4)
   real(r8)::rbuf1(NX,4*(endlev-beglev+1)+4)
...
!   DO J  = beglatdyn, endlatdyn
!
! for extended vertical variables
!      DO K = beglev+1 ,endlev+1
!         DO I = 1 ,NX
            UZ(I,K,J) = UT(I,K-1,J)
            VZ(I,K,J) = VT(I,K-1,J)
...
!!#if (defined SPMD)
!JJR WSstar(k+1) may not need communicate

!!      call mpi_move_left(Vstar(1,1,endlatdyn),Vstar(1,1,beglatdyn-1),NX*NL)
!!      call mpi_move_left(   TT(1,1,endlatdyn),   TT(1,1,beglatdyn-1),NX*NL)
!!      call mpi_move_right(    TT(1,1,beglatdyn),   TT(1,1,endlatdyn+1),NX*NL)
!!#endif
...
!!   PRINT *, 'run time of FILT2D-T was ', time_end - time_begin, 'sec'
!----------------------------------------------------------------
!
!     *******************************************************************
!     *******************************************************************
...
!jjr   call mpi_move_left(Vstar(1,1,endlatdyn), Vstar(1,1,beglatdyn-1),NX*NL)
!jjr   call mpi_move_left(   UT(1,1,endlatdyn),    UT(1,1,beglatdyn-1),NX*NL)
!jjr   call mpi_move_right( Ustar(1,1,beglatdyn), Ustar(1,1,endlatdyn+1),NX*NL)
!jjr   call mpi_move_right(    VT(1,1,beglatdyn),    VT(1,1,endlatdyn+1),NX*NL)
!jjr   call mpi_move_left(   VT(1,1,endlatdyn),    VT(1,1,beglatdyn-1),NX*NL)
!jjr   call mpi_move_right( Vstar(1,1,beglatdyn), Vstar(1,1,endlatdyn+1),NX*NL)
!!   call mpi_move_right(WSstar(1,1,beglatdyn),WSstar(1,1,endlatdyn+1),NX*NL)
!jjr   call mpi_move_right(WSstar(1,1,beglatdyn),WSstar(1,1,endlatdyn+1),NX*NZ)   !zhh
!right
...
!jjr      DO K = 2 ,NZ
!jjr         DO I = 1 ,NX
!jjr            UZ(I,K,J) = UT(I,K-1,J) 
!jjr            VZ(I,K,J) = VT(I,K-1,J)
!jjr         ENDDO
!jjr      ENDDO
!jjr    ENDDO  !jjr
      DO J  = beglatdyn, endlatdyn !jjr
...
!=============================== zhh =====================================
      DO K = beglev ,endlev
         DO I = 1 ,NX
            IF (abs(DU(I,K,J)) > 1E1 .OR. abs(DV(I,K,J)) > 1E1) THEN
               print*, 'DU(',I,K,J,') =', DU(I,K,J),i,k,j
