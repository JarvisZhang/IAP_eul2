...
! Completed : 2005.9.15
! Update: 2007.12.22, ZhangHe, add Qliq, Qice in sub. smooth
!         2008.4.7, ZhangHe, add caculation of Psa in sub. SMOTHP
!         2008.5, Wujianping, parallel version
!         2008.6.11, ZhangHe, available for both serial & parallel
!         2008.6.14, ZhangHe, add 'Istar' in sub. SMOTHP
!------------------------------------------------------------------------------------------------
   ...
   use pmgrid,     only: beglatdyn, endlatdyn, beglatdynex, endlatdynex, &
                 loc_JB, loc_JE,beglev,endlev,iam,npr_y,iam
#if ( defined SPMD )
...
!------------------------------------------------------------------------------------------------
!dry      use surface, only: TA
      use IAP_prog,   only: U, V, T, Q, Qliq, Qice
      implicit none
!-------------------------------------Local workspace-------------------------------------------
...
!------------------------------------------------------------------------------------------------
!      DO J = loc_JB,loc_JE
!jjr no need      DO J = beglatdyn, endlatdyn
      DO J = beglatdyn,endlatdyn 
...
!-----------------------------------------------------------------------------------------------
!=============================== zhh =====================================
!!      print*, 'at the beginning of SOSS2D  ', 'A(4,1) =', A(4,1), 'A(4,2) =', A(4,2)
!============================ 2007.8.8 ====================================
!
...
!jjr      call mpi_move_left(B(1,endlatdyn-1), B(1,beglatdyn-2), 4*IG)
      ...
      call mp_send3d( dest, src, IG, JG,1,                     &
                      1, IG, 1,JG ,1,1,       &
                      1, IG, endlatdyn-1, endlatdyn+2, 1, 1, B )
      call mp_recv3d( src, IG,  JG,1,                             &
                      1, IG, 1, JG,1,1,       &
