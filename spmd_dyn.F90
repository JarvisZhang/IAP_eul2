...
! !USES:
   ...
   use pmgrid
!jjr, only: plat, plon, masterproc, iam, numbnd, &
...
!                     twod_decomp, spmd_on, mod_transpose, mod_geopk
 ...
 !jjr  use scanslt,      only: beglatex, endlatex, numbnd
...
! !PUBLIC DATA MEMBERS:
   ...
   type (parpatterntype)   :: ikj_xy_to_yz,ikj_yz_to_xy, ijk_yz_to_xy, &
...
!$omp end parallel
     ...
     if (present(force_2d_in) ) then
        if (npr_z .eq. 1 .and. nprxy_x .eq. 1 .and. force_2d_in .eq. 0) then
           twod_decomp = 0
           if (masterproc) then
              write (6,*) 'decomposition is effectively 1D - skipping transposes'
           endif
        else
           if (masterproc) then
              write (6,*) 'using multi-2d decomposition methodology'
           endif
        endif
     else
...
!               beglatex= beglat-numbnd !jjr
!               endlatex=endlat+numbnd !jjr
...
! Compute local limits
      ...
      beglatxy = 1
      endlatxy = ydistxy(1)
!jjr       print*,'test endlatxy',endlatxy,nprxy_y
      do procid = 1, myidxy_y
         beglatxy = endlatxy + 1
         endlatxy = beglatxy + ydistxy(procid+1) - 1
      enddo
!               beglatxyex= beglat
!               endlatxyex=endlat+2*numbnd
              beglatxydyn = plat+1-endlatxy
...
!jjr      deallocate (ydist)
      zdist(npr_z)=zdist(npr_z)-1
!jjr      deallocate (zdist)
