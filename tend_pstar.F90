...
! Author : ZhangHe
! Completed : 2005.9.3
! Update    : 2006.3.25, ZhangHe, add two methods to define flexible substitute (Istar = 3 & 4)
!             2007.4.23, ZhangHe, delete dumb parameter 'SETUV'
!             2007.5.07, ZhangHe, change subroutine's name from pstartend to tend_pstar
...
!------------------------------------------------------------------------------------------------
   ...
   use pmgrid,     only: beglatdyn, endlatdyn, loc_JB, loc_JE,npr_z,&
               beglatdynex,endlatdynex,beglev,endlev,endlevp,iam,npr_y
#if ( defined SPMD )
...
!----------------------------------Local workspace-----------------------------------------------
   real(r8) :: PXW3(NX,NY), PXW4(NX,NY)      ! auxiliary variables
   real(r8) :: PYP2(NX,NY), PYM2(NX,NY)      ! auxiliary variables
   real(r8) :: DIVX2, DIVY2, DIV2(IB:IE,NL,beglatdyn:endlatdyn)  ! divergence of [(pes*)¡¤(v*)]
...
#if (defined SPMD)
!jjr      call mpi_move_left(P(1,endlatdyn), P(1,beglatdyn-1),NX)
!jjr      call mpi_move_right( P(1,beglatdyn), P(1,endlatdyn+1),NX)
      call mp_send3d( dest, src, NX,NY,1,                     &
                      1, NX, beglatdynex, endlatdynex,1,1,       &
