...
!            2007.04.20, ZhangHe, add 'NPHF', 'NENG' & 'NFST'
!            2007.05.23, ZhangHe, update 'PHALFC26' according to initial data of CAM3.1
!            2007.12.22, ZhangHe, reset DTIMEQ
!            2008.04.07, ZhangHe, add statement of Kstd
!            2008.04.10, ZhangHe, add statement of Istar
!            2010.08.10, ZhangHe, add Ndq and modify DTIMEQ
...
!!   real(r8), parameter :: DTPHYS = 1800.0D0   ! timestep of physical package
...
!-----------------------------------------------------------------------------------------------
      use pmgrid,     only: beglatdyn, endlatdyn, masterproc, iam
      implicit none
#include <comctl.h>   ! adiabatic
!----------------------------------Local workspace----------------------------------------------
      integer :: i,j,k
!-----------------------------------------------------------------------------------------------
!!      if (masterproc) print*, 'At beginning of sub. setfle '
!!      print*, 'iam =', iam, 'beg=', beglatdyn, 'end=', endlatdyn
!   set the pressure at sigma level according to vertical level NLAY 
      do K = 1, NZ                       
         if ( NL == 19 ) then
            PHALFC(K) = PHALFC19(K)
         else if ( NL == 26 ) then
